...
from models.base.base_sampler import VariableSampler, batch_by_size
...
class BaseTrainer:
    ...
    def _load_model(
        ...
        if resume_type in ["resume", ""]:
            ...
            if hasattr(self.cfg.train, "resume_lr_scheduler") and self.cfg.train.resume_lr_scheduler is False:
                self.step = 0
                self.scheduler = self._build_scheduler()
                self.scheduler = self.accelerator.prepare(self.scheduler)
                if self.accelerator.is_main_process:
                    self.logger.info("Reinitialized scheduler to start from step 0...")
