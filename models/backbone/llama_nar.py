...
class LlamaNARDecoderLayer(LlamaDecoderLayer):
    #same as dit but without dropath
    #x = x + self.droppath(self.attention(self.norm1(x), pos))
...
class DiffLlama(LlamaModel):
    def __init__(
        ...
        self.latent_out_mlp = nn.Sequential(
            nn.Linear(hidden_size, hidden_size * 4),
            nn.SiLU(),
    ...
    def forward(
        ...
        diffusion_step,
        x_mask,
        timeframe_cond: Optional[torch.Tensor] = None,
        ...
        #now global_cond is clap embedding(b, 1024)
        if global_cond is not None:
            diffusion_step = diffusion_step + global_cond
        ...
        hidden_states = self.latent_out_mlp(hidden_states)
