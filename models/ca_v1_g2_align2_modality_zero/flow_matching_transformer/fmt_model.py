...
from models.layers.transformer_decoder import Transformer_Decoder, AA_Transformer_Decoder, CA_Transformer_Decoder, DiT
...
class FlowMatchingTransformer(nn.Module):
    def __init__(
        ...
        self.av_transformer = CA_Transformer_Decoder(**cfg.av_transformer.arch)
        ...
        self.ir_decoder = nn.Linear(self.hidden_size, self.hidden_size)
        if self.aligner_activate:
            self.aligner_av_transformer = DiT(
                dim = self.hidden_size,
                depth = 1,
                heads = 4,
                dim_head = self.hidden_size // 4,
                dropout = 0.1,
                ff_mult = 2,
            ...
            self.align_out_proj = nn.Linear(self.hidden_size, 8)
    ...
    def prepare_tokens(self, ir_z, depth_map, loc, irs_num, bsz):
        ...
        if self.aligner_activate:
            tgt_ir_init_tokens = tgt_ir_init_tokens.squeeze(1)# (B, T, dim)
            tgt_label_tokens = label_tokens[:, -1, 0]#(B, dim)
            tgt_ir_init_tokens = self.aligner_av_transformer(tgt_ir_init_tokens, tgt_label_tokens)#(B, T, dim)
            align_feature = self.align_out_proj(tgt_ir_init_tokens)#(B, T, 8)
            tgt_ir_init_tokens = tgt_ir_init_tokens.unsqueeze(1)#(B, 1, T, dim)
            ir_tokens = torch.cat([ref_ir_tokens, tgt_ir_init_tokens], dim = 1) #(B, N, 1, dim)
            return label_tokens, ir_tokens, align_feature
        else:
            ir_tokens = torch.cat([ref_ir_tokens, tgt_ir_init_tokens], dim = 1) #(B, N, 1, dim)
    ...
    def aggregator(self, ir_tokens, label_tokens):
        ...
        context = torch.cat([label_tokens, ir_tokens], dim = -2)#(B, N, T+1, D)
        x = context[:,-1] 
        c = rearrange(context[:,:-1], "b n t d -> b (n t) d")
        mask = torch.ones(B, N, T+1, device = ir_tokens.device)
        mask[:, :-1, 1:] = 0

        visiable_reference_geo_token = 4
        mask[:, :visiable_reference_geo_token, 1:] = 1
        mask = mask.bool()
        x_mask = mask[:, -1]
        c_mask = mask[:, :-1]
        c_mask = rearrange(c_mask, "b n t -> b (n t)", n = N-1)
        
        out = self.av_transformer(x, c, x_mask = x_mask, c_mask = c_mask)
        out = out.unsqueeze(1)
        c = rearrange(c, "b (n t) d -> b n t d", n = N-1)
        ref_ir_tokens = c[:,:,1:]
        ref_label_tokens = c[:,:,0:1]
        tgt_ir_tokens = out[:,:,1:]
        tgt_label_tokens = out[:,:,0:1]
        ir_tokens = torch.cat([ref_ir_tokens, tgt_ir_tokens], dim = 1)
        label_tokens = torch.cat([ref_label_tokens, tgt_label_tokens], dim = 1)
        
        return ir_tokens, label_tokens
    ...
    def forward(self, ir_z, cc_depth_map, cc_src_loc, align_feat_ir = None):
        ...
        if self.aligner_activate:
            label_tokens, ir_tokens, align_feature = self.prepare_tokens(ir_z, cc_depth_map, cc_src_loc, irs_num, bsz)
        else:
        ...
        # print("--------------------------------")
        # #exit()
        ir_tokens, label_tokens = self.aggregator(ir_tokens, label_tokens)
        tgt_ir_tokens = ir_tokens[:, -1]#(B, T, D)
        out_tgt_ir_tokens = self.ir_decoder(tgt_ir_tokens)#(B, T, D)
        
        
        if self.aligner_activate:
            return out_tgt_ir_tokens, align_feature
        else:
            return out_tgt_ir_tokens
