...
from models.aa_v1_g2_align2.flow_matching_transformer.fmt_model import FlowMatchingTransformer
...
class FMTTrainer(BaseTrainer):
    def __init__(self, args, cfg):
        ...
        self.aligner_codebook_depth = self.cfg.model.flow_matching_transformer.aligner.codebook_depth
    ...
    def _build_dataset(self):
        return AcousticRooms_Dataset, AcousticRooms_Collator
    ...
    def get_align_features(self, audio, layer = None): # 
        """
        args:
        audio:(B*N, 1, T)
        """
        # codes = self.audio_codec.encode(audio, layer)#(B*N, layer, t)
        # align_features = self.audio_codec.model.quantizer.from_codes(codes)[0]#(B*N, 1024, t)
        # align_features = align_features.transpose(-1, -2)#(B*N, t, 1024)
        
        align_features = self.audio_codec.encode_z(audio)#(B*N, t, 1024)
        align_features = align_features.transpose(-1, -2)#(B*N, 1024, t)
    ...
    def get_align_loss(self, x, y):
        ...
        align_loss:(1,)
        """
        assert x.shape == y.shape, "The shape of x and y must be the same"
        alignment_loss = torch.mean(1 - F.cosine_similarity(x, y, dim = -1))
    ...
    def _train_step(self, batch):
        ...
        align_loss = torch.zeros(1).to(self.accelerator.device)
        if not self.aligner_activate:
            ...
            gt_align_features = self.get_align_features(gt_env, layer = self.aligner_codebook_depth) #(B*N, D)
            align_loss = self.get_align_loss(pred_align_features, gt_align_features)
        ...
        alignment_loss = self.cfg.loss.align_loss_weight * align_loss
        auxiliary_loss_weight_scale = self.loss_weight_warmup_factor(type = 'auxiliary')
        batch_total_loss = main_loss + alignment_loss\
        ...
        train_losses["align_loss"] = align_loss
