...
from models.layers.utils import is_package_available
...
class AttnProcessor:
    ...
    def get_attn_weight(self, query, key, attn_mask=None):
        # SDPA 
    ...
    def __call__(
        ...
        if self.return_attn_weight:
            return x, self.get_attn_weight(query, key, attn_mask).detach()
        else:
            return x
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, embedding, mask=None, rope=None):  # x: noised input, embedding: time embedding + label embedding
        # pre-norm & modulation for attention input
        norm, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.attn_norm(x, emb=embedding)
...
class SoftEmbedding(nn.Module):
    """
    W: (vocab_size, emb_dim) learnable
    input coeff p: (..., vocab_size) float, e.g. in [0,1] or probability simplex
    output: (..., emb_dim)
    """
    def __init__(self, vocab_size: int, 
        ...
        self.weight = nn.Parameter(torch.empty(vocab_size, emb_dim))
        self.reset_parameters(init=init)

    def reset_parameters(self, init: str = "uniform"):
        bound = 1.0 / math.sqrt(self.emb_dim)
        if init == "uniform":
            nn.init.uniform_(self.weight, -bound, bound)
        elif init == "normal":
            nn.init.normal_(self.weight, mean=0.0, std=bound)
    ...
    def forward(self, coeff: torch.Tensor) -> torch.Tensor:
        """
        coeff: shape (..., vocab_size)
        returns: shape (..., emb_dim)
        """
        if coeff.shape[-1] != self.vocab_size:
            raise ValueError(f"coeff last dim must be vocab_size={self.vocab_size}, got {coeff.shape[-1]}")
        return coeff @ self.weight
...
class CrossAttention(nn.Module):
    def __init__(
        ...
        self.context_dim = (dim if context_dim is None else context_dim)
        self.context_pre_only = context_pre_only
        
        self.to_q = nn.Linear(dim, self.inner_dim)
        self.to_k = nn.Linear(self.context_dim, self.inner_dim)
        self.to_v = nn.Linear(self.context_dim, self.inner_dim)
    ...
    def forward(
        ...
        x_rope=None,  # rotary position embedding for x
        c_rope=None,  # rotary position embedding for c
    ) -> torch.Tensor:
        return self.processor(self, x, c=c, x_mask=x_mask, c_mask=c_mask, x_rope=x_rope, c_rope=c_rope)
...
class CrossAttnProcessor:
    ...
    def __call__(
        self,
        attn: CrossAttention,
        x: float["b nx d"],  # noised input x
        c: float["b nc d"] = None,  # context c, here text
        x_mask: bool["b nx"] | None = None,
        c_mask: bool["b nc"] | None = None,
        x_rope=None,  # rotary position embedding of x
        c_rope=None,  # rotary position embedding of c
    ) -> torch.FloatTensor:
        batch_size = x.shape[0]
        if x_mask is None:
            x_mask = torch.ones((batch_size, x.shape[1]), device=x.device, dtype = torch.bool)
        if c_mask is None:
            c_mask = torch.ones(batch_size, c.shape[1], device=c.device, dtype = torch.bool)
        # `sample` projections
        query = attn.to_q(x)
        key = attn.to_k(c)
        value = attn.to_v(c)
        ...
        # apply rotary position embedding
        if x_rope is not None:
            freqs, xpos_scale = x_rope
            q_xpos_scale, k_xpos_scale = (xpos_scale, xpos_scale**-1.0) if xpos_scale is not None else (1.0, 1.0)
            if self.pe_attn_head is not None:
                pn = self.pe_attn_head
                query[:, :pn, :, :] = apply_rotary_pos_emb(query[:, :pn, :, :], freqs, q_xpos_scale)
                ...
                query = apply_rotary_pos_emb(query, freqs, q_xpos_scale)
        if c_rope is not None:
            freqs, cpos_scale = c_rope
            q_cpos_scale, k_cpos_scale = (cpos_scale, cpos_scale**-1.0) if cpos_scale is not None else (1.0, 1.0)
            if self.pe_attn_head is not None:
                pn = self.pe_attn_head
                key[:, :pn, :, :] = apply_rotary_pos_emb(key[:, :pn, :, :], freqs, k_cpos_scale)
            else:
                key = apply_rotary_pos_emb(key, freqs, k_cpos_scale)
 

        if self.attn_backend == "torch":
            # mask. e.g. inference got a batch with different target durations, mask out the padding
            if self.attn_mask_enabled:
                q_mask = x_mask.unsqueeze(1).unsqueeze(-1)  # 'b n -> b 1 n 1'
                k_mask = c_mask.unsqueeze(1).unsqueeze(1)  # 'b n -> b 1 1 n'
                attn_mask = q_mask & k_mask
            else:
                attn_mask = None
            x = F.scaled_dot_product_attention(query, key, value, attn_mask=attn_mask, dropout_p=0.0, is_causal=False)
        ...
        if c_mask is not None:
            ...
            c = c.masked_fill(~c_mask, 0.0)
        if self.return_attn_weight:
            raise ValueError("return_attn_weight is not supported for CrossAttention")
        else:
            return x
