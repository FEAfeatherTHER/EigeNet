...
class FMTTrainer(BaseTrainer):
    ...
    def _train_step(self, batch):
        ...
        pred_tgt_z = self.model(
            all_ir_z,
            cc_depth_map,
            all_cc_src_loc,
        )
        if torch.isnan(pred_tgt_z).any() or torch.isinf(pred_tgt_z).any():
            print(f"*"*20)
            print("存在Nan或Inf")
            exit()
        
        pred_tgt_ir = self.decode_z_to_audio(pred_tgt_z.transpose(-1, -2))
        # print(f"*"*20)
        # print(f"检查输出")
        ...
        energy_loss, mrstft_loss, energy_decay_loss = self.waveform_loss_fn(pred_tgt_ir, gt_tgt_ir)
        batch_total_loss = self.cfg.loss.energy_loss_weight * energy_loss \
            + self.cfg.loss.mrstft_loss_weight * mrstft_loss\
            + self.cfg.loss.energy_decay_loss_weight * energy_decay_loss
        total_loss += batch_total_loss
        train_losses["batch_total_loss"] = batch_total_loss
        train_losses["energy_loss"] = energy_loss
        train_losses["mrstft_loss"] = mrstft_loss
        train_losses["energy_decay_loss"] = energy_decay_loss
