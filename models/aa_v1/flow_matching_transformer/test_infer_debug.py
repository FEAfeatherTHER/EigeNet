...
class testset_acousticrooms_dataset(Dataset):
    ...
    def get_batch(self, segment_info):
        ...
        if len(other_src_idx_list) == 0:
            ...
            all_code_mask = frame2mask(all_ir_frames, max_frame)

            return {
                "all_ir": all_ir, #(N, t)
                "all_cc_src_loc": all_cc_src_loc, #(N, 3)
                "cc_depth_map": cc_depth_map, #(256, 512, 3)
                "all_code_mask": all_code_mask, #(N,)
...
def main_debug(args):
    ...
    test_jsonl_list = cfg.dataset.test_rir_list
...
if __name__ == "__main__":
    fmt_cfg = os.path.join(src, f"egs/rir/flow_matching_transformer/debug_EigeNet_v2_1.json")
    fmt_ckpt = "/data/250010171/ckpts/EigeNet/mgm_dit_v2_1/debug/checkpoint_backup/epoch-0159_step-0120000_loss-4.300866"
