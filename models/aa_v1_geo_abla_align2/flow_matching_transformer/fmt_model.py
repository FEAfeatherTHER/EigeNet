...
class FlowMatchingTransformer(nn.Module):
    def __init__(
        ...
        self.abla_depth = cfg.geo_abla.depth
        self.abla_loc = cfg.geo_abla.loc
        ...
        if not self.abla_depth or not self.abla_loc:
            self.geo_proj = nn.Linear(self.depth_dim, self.hidden_size)
    ...
    def prepare_tokens(self, ir_z, depth_map, loc, irs_num, bsz):
        ...
        if self.abla_depth:
            label_tokens = self.geo_proj(geo_tokens)
        elif self.abla_loc:
            label_tokens = self.geo_proj(loc_tokens)

        time_tokens = self.get_time_tokens(self.ir_time_interval) #(T, dim)
        tgt_ir_init_tokens = time_tokens.unsqueeze(0).unsqueeze(0).repeat(label_tokens.shape[0], 1, 1, 1) #(B, 1, T, dim)
