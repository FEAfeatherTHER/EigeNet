...
def main_debug(args):
    ...
    for test_jsonl in test_jsonl_list:
            ...
            for batch_idx in tqdm(range(0, len(dataset), bsz)):
                ...
                for i in range(present_bsz):
                    ...
                    if segment is None or valid_src_num < dynamic_reference_count:
                        print(f"valid_src_num: {valid_src_num} < dynamic_reference_count: {dynamic_reference_count}, skip this batch")
                        continue
                    else:
        ...
        metric_path = os.path.join(output_folder, f"{duration}_metrics_sup.csv")
        log.to_csv(metric_path, index = False)
        
if __name__ == "__main__":

    # # # only depth
    # fmt_cfg = '/data/250010171/code/EigeNet_discriminant/egs/rir/flow_matching_transformer/debug_EigeNet_v1_only_depth_aa_align2.json'
    # fmt_ckpt = "/data/share/amphion/data/eigenet/ckpts/discriminant/ablation_only_depth_noalign/checkpoint/epoch-0009_step-0007600_loss-2.141709"
    
    # only loc
    fmt_cfg = '/data/250010171/code/EigeNet_discriminant/egs/rir/flow_matching_transformer/debug_EigeNet_v1_only_loc_aa_align2.json'
    fmt_ckpt = "/data/share/amphion/data/eigenet/ckpts/discriminant/ablation_only_loc_noalign/checkpoint/epoch-0009_step-0007600_loss-2.337970"
