...
class FMTTrainer(BaseTrainer):
    def __init__(self, args, cfg):
        ...
        self.waveform_loss_fn = waveform_loss_fn(
            ...
            env_normalized=self.cfg.loss.env.normalized,
    ...
    def loss_weight_warmup_factor(self):
        """calculate the linear weight factor for the loss weights"""
        if self.step < self.cfg.train.lr_warmup_steps:
    ...
    def _train_step(self, batch):
        ...
        if not self.cfg.model.flow_matching_transformer.env:
            ...
            pred_tgt_env = self.decode_z_to_audio(pred_tgt_env_z.transpose(-1, -2))
        ...
        auxiliary_loss_weight_scale = self.loss_weight_warmup_factor()
        batch_total_loss = main_loss + auxiliary_loss_weight_scale * auxiliary_loss
        ...
        train_losses["time_edc_loss_weight"] = self.cfg.loss.time_edc_loss_weight * auxiliary_loss_weight_scale
        train_losses["spect_edc_loss_weight"] = self.cfg.loss.spect_edc_loss_weight * auxiliary_loss_weight_scale
        train_losses["env_loss_weight"] = self.cfg.loss.env_loss_weight * auxiliary_loss_weight_scale
