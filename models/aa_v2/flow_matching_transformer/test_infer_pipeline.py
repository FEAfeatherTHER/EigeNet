...
from models.aa_v2.base.dac_codec import DAC
...
class InferencePipeline:
    ...
    def inference_fm(
        ...
        with torch.no_grad():
            if self.enable_env:
                predict_env_z, predict_ir_z = self.fmt_model.forward(all_ir_z, cc_depth_map, all_cc_src_loc)
            else:
            ...
            elif self.enable_env and return_env:
                predict_env_z = predict_env_z.transpose(1, 2) #(b, t, d) -> (b, d, t)
                synthesized_env = self.decode_z_to_audio(predict_env_z)
                return synthesized_audio.cpu().numpy(), synthesized_env.cpu().numpy()
