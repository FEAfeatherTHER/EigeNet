...
class FlowMatchingTransformer(nn.Module):
    def __init__(
        ...
        if self.aligner_activate:
            ...
            Mlp(pre_dim, self.hidden_size, self.hidden_size))
            self.aligner_label_proj = nn.Linear(self.hidden_size, self.hidden_size)
    ...
    def aggregator(self, ir_tokens, label_tokens):
        ...
        out = self.av_transformer(context, mask = mask)
    ...
    def forward(self, ir_z, cc_depth_map, cc_src_loc, align_feat_ir = None):
        ...
        if self.aligner_activate:
            ...
            label_tokens = rearrange(label_tokens, "b n d -> (b n) d")#(B*N, D)
            label_tokens = self.aligner_label_proj(label_tokens)#(B*N, D)
            env_tokens = self.env_time_interval_embedding(self.env_time_interval)#(T, D)
