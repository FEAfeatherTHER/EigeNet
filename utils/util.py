...
class JsonHParams:
    ...
    def to_dict(self):
        """递归地将 JsonHParams 对象转换回原生 dict"""
        d = {}
        for k, v in self.items():
            if isinstance(v, JsonHParams):
                d[k] = v.to_dict()
            else:
                d[k] = v
        return d
...
import librosa
def vad(audio_file, start, end, sample_rate=48000, silence_threshold=0.01, silence_ratio=0.9):
    """
    检测音频是否80%都是空白的（无声部分）。
    ...
    # 加载音频文件
    duration = end - start
    audio, sr = librosa.load(audio_file, sr=sample_rate, offset=start, duration=duration, mono = True)
    ...
    if silence_percentage > silence_ratio:
        #sf.write(f"/data/250010171/workspace/jingchong/data/debug/bug.wav", audio, sr)
        #sf.write(f"/data/250010171/workspace/jingchong/data/debug/bug_norm.wav", normalized_amplitude, sr)
